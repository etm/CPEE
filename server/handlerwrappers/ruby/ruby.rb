...
module HandlerWrapper
  module Ruby
    ...
    DSL_TO_DSLX_XSL  = File.expand_path(File.join(__dir__,'transformation_dslx.xsl'))
    ...
    def self::dslx_to_dsl(dslx) # transpile
      trans = XML::Smart::open_unprotected(File.join(__dir__,HandlerWrapper::Ruby::DSL_TO_DSLX_XSL))
    ...
    def self::prepare(id,opts) # write result to disk
      ...
      FileUtils.copy(HandlerWrapper::Ruby::BACKEND_RUN,File.join(opts[:instances],id.to_s))
      ...
      File.open(File.join(opts[:instances],id.to_s,HandlerWrapper::Ruby::BACKEND_OPTS),'w') do |f|
        YAML::dump({
          :host => opts[:host],
          :url => opts[:url],
      ...
      template = ERB.new(File.read(HandlerWrapper::Ruby::BACKEND_TEMPLATE), trim_mode: '-')
      res = template.result_with_hash(dsl: dsl, dataelements: dataelements, endpoints: endpoints, positions: positions)
      File.write(File.join(opts[:instances],id.to_s,HandlerWrapper::Ruby::BACKEND_INSTANCE),res)
    end

    def self::run(id,opts)
      exe = File.join(opts[:instances],id.to_s,File.basename(HandlerWrapper::Ruby::BACKEND_RUN))
