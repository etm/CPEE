...
module CPEE
  ...
  module ExecutionHandler
    ...
    module Rust
      BACKEND_INSTANCE = File.expand_path(File.join(__dir__,'backend/rusty_weel','instance.rs'))
      BACKEND_OPTS     = File.expand_path(File.join(__dir__,'backend/rusty_weel/configs/','opts.json'))
      BACKEND_CONTEXT  = File.expand_path(File.join(__dir__,'backend','context.json'))
      BACKEND_COMPILE  = File.expand_path(File.join(__dir__,'backend/rusty_weel/scripts/cpee','compile'))
      BACKEND_RUN      = File.expand_path(File.join(__dir__,'backend/rusty_weel/scripts/cpee','run'))
      INDENT = 4
      p BACKEND_INSTANCE
      module Translate #{{{
