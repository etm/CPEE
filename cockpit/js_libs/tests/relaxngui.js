...
var RelaxNGui = function(rng,target,ceval,ignore=false) {
  ...
  var addelements = function(target,container) { //{{{
    ...
    template.find('> [data-type=mov]').moveitem(template);
  ...
  var recshow_single = function(tag,ret,template,path,lencount,optional){ //{{{
    ...
    $.each($(tag).children('choice[href]'), function(k,v) {
  ...
  (function($) { //{{{
    $.fn.moveitem = function(item) {
      ...
      $(document).on("mousemove", function(e) {
        ...
        if (prev.length > 0 && e.originalEvent.clientY < prev.position().top + prev.outerHeight()) {
          $(prev).before($(item));
          pos = e.originalEvent.clientY;
        } else if (next.length > 0 && e.originalEvent.clientY > next.position().top) {
          $(next).after($(item));
          pos = e.originalEvent.clientY;
        }
